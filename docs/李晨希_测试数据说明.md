...
> 本文记录 `tests/conftest.py` 中李晨希 fixture 的设计依据。正式提交前需结合本人实际测试设计进行核对和改写。

## 1. 本阶段目的

本阶段只建立可复用、可追踪的小尺寸测试数据，不编写具体测试步骤和断言。这样可以把“准备输入数据”和“设计测试用例”分开，减少重复代码，并保证两次运行得到相同结果。

李晨希的 fixture 统一使用以下前缀：
...
工厂允许覆盖 batch、head、序列长度和 head dimension，以便后续由李晨希自行构造不同规模的输入。
...
这与 `graph_rank_separators()` 只允许右侧 padding 的当前实现一致，也能让李晨希后续自行观察完整序列与右侧 padding 序列之间的差异。
...
5. 后续新增 fixture 时继续使用李晨希负责部分的命名前缀。
...
随后使用隔离目录中的 Transformers 4.50.3 和 Tokenizers 0.21.4 复核，`MMSepCache` 能正常创建，所有固定 fixture 的形状也符合本说明。因此项目依赖基线改为这两个版本。该记录属于环境兼容性结论，不计作李晨希的功能缺陷。
