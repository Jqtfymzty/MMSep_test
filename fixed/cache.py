...
class MMSepCache(Cache):
    ...
    def __init__(self,                               
        ...
        # Newer Transformers requires either ``layers`` or
        # ``layer_class_to_replicate`` when constructing Cache.  MMSepCache
        # maintains its own key/value lists, so an empty layer list is enough
        # for the base class; the fallback keeps compatibility with older
        # Transformers releases whose constructor took no arguments.
        try:
    ...
    def update_kv_cache_and_past_tok_ids(self, new_kv_pair: Tuple[torch.Tensor], input_ids: torch.Tensor, layer_idx: int, COMPRESS_KV=False, SEP_ACCUMULATION:bool=True, USE_MAX_SEP_CACHE:bool=False, SEP_PADDING_IN_BATCH:bool=True) -> None:
        ...
        # 4. 鍚庣画decode闃舵锛屾瘡娆¤緭鍏ヤ竴涓猼oken锛宎ppend鍒癱ache鍚庨潰锛屽苟璁板綍 gen_token 鐨?id 鍜岄暱搴?
        if input_ids is not None:
            self.append_past_tok_ids(input_ids, layer_idx)  # 杩欓噷 append 鏄綋鍓嶈В鐮?step 鐨?input_ids锛屽彧鑳藉湪杩欎簺 token 閲屼涪寮冮潪 sep token 鐨?kv
        # kv cache 涓涓㈠純鐨?token 浣嶇疆鏄紝past_tok_ids 涓潪sep 鐨勪綅缃?+ init_cache_size
